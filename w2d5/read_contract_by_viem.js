...
const contract = getContract({ address, abi, client: client })

const tokenId=1

// The below will send a single request to the RPC Provider.
const [tokenUrl,ownerOf_tokenId] = await Promise.all([

  contract.read.tokenURI([tokenId]),
  contract.read.ownerOf([tokenId])
])

console.log("tokenUrl of token 1:",tokenUrl)
console.log("owner of token 1:",ownerOf_tokenId)
